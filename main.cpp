...
int main(){
    ...
    Scheduler My_scheduler = Scheduler(s, false);
    int q=quantum;
    // the main while loop should be changed to take flag stop or stth from GUI
    for (int i = 0;i < 30;i++) {
        My_scheduler.run_scheduler_once(ArrivalBuffer,q);
        q--;
        if(q==0)q=quantum;
    }
    cout<<"\navgTurnAround "<<<My_scheduler.getAvgTurnAround()/n<<endl;
    cout<<"avgWaitingTime "<<My_scheduler.getAvgWaitingTime()/n<<endl;
