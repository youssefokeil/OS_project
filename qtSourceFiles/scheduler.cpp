...
void Scheduler::run_scheduler_once(priority_queue<Process, std::vector<Process>, CompareByArrivalTime> &PassedArrivalBuffer,MainWindow *mainWindow,int &q) {
    ...
    const string& str = gantt_chart.back();
    QLabel *newLabel = new QLabel("New Label", mainWindow);
    newLabel->setText(QString::fromStdString(str));

    if(str=="idle"){
        newLabel->setStyleSheet(
            "QLabel {"
            "   color: white;"
            "   background-color: #5b4965;"  // Light Purple
        ...
        newLabel->setStyleSheet(
            "QLabel {"
            "   color: white;"
            "   background-color: #3d354b;"  // Nice purple
            "   border: 1px solid #2e293a;"
            "   border-radius: 2px;"
            "   padding: 1px 3px;"
            "   qproperty-alignment: 'AlignCenter';"
    ...
    mainWindow->getHorizontalLayout()->addWidget(newLabel);

    if(!gantt_chart.empty() && str!="idle"){
        int id=str[1]-'0';
        QTableWidgetItem *item = new QTableWidgetItem(QString::number(mainWindow->getTableWidget()->item(id-1,0)->text().toInt()-1));
        mainWindow->getTableWidget()->setItem(id-1,0,item);
